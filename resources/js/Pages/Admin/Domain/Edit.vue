    ...
    <Admin>
            ...
            <ol class="inline-flex items-center space-x-1 md:space-x-3 rtl:space-x-reverse">
                ...
                <li>
                    <div class="flex items-center">
                        ...
                        <Link :href="route('admin.domains.index')"
        ...
        <div class="flex justify-between">
            <h2 class="mb-4 text-3xl font-extrabold leading-none tracking-tight text-gray-900 md:text-4xl">
                Update Locking Domain
            </h2>
        </div>


        <div class="grid grid-cols-1 gap-6 sm:grid-cols-1">
            <form @submit.prevent="form.put(route('admin.domains.update', domain.id))" class="pb-10">
                <div class="mb-5">
                    <label class="block mb-2 text-sm font-medium text-green-700"
                           for="domain-success">Domain</label>
                    <input v-model="form.domain" id="domain" class="border text-green-900 placeholder-green-700 text-sm rounded-lg block w-full p-2.5"
                           placeholder="Domain"
                           type="text" required>
                </div>
                <div class="mb-5">
                    ...
                    <input v-model="form.status" id="status" class="border text-green-900 placeholder-green-700 text-sm rounded-lg block w-full p-2.5"
                           placeholder="Status"
                           type="text" required>
                </div>
